...
//	}
//	else if (source_addr.sin_addr.s_addr == session.server.dest_addr.sin_addr.s_addr)
//	{
...
//		session.server.send_addr = source_addr;
//	}
//
...
//
//    session.server.dest_addr.sin_addr.s_addr = htonl(INADDR_ANY);
//    session.server.dest_addr.sin_family = AF_INET;
//    session.server.dest_addr.sin_port = htons(CONFIG_DEST_PORT); //TODO: rename this
//    session.server.addr_family = AF_INET;
//    session.server.ip_protocol = IPPROTO_IP;
//    inet_ntoa_r(session.server.dest_addr.sin_addr, session.server.addr_str, sizeof(session.server.addr_str) - 1);
//
...
//
//    int err = bind(session.server.sock, (struct sockaddr *)&session.server.dest_addr, sizeof(session.server.dest_addr));
//    if (err < 0) {
...
//	}
//
//	int err = sendto(session.server.sock, payload, size, 0, (struct sockaddr *)&session.server.send_addr, sizeof(session.server.send_addr));
//	if (err < 0) {
//		ret_val = ESP_FAIL;
